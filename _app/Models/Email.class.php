...
//    }
    private function setMsg($template = '') {
        if ($template != ''):
            //$file = './../../_tpl/' . $template;
            $file = './_tpl/' . $template;
            $AbrirArquivo = fopen($file, "r");
            $EmailConteudo = fread($AbrirArquivo, filesize($file));
            $EmailConteudo = str_replace("%%NOME%%", $this->DestinoNome, $EmailConteudo);
            $EmailConteudo = str_replace("%%MENSAGEM%%", $this->Mensagem, $EmailConteudo);
            
            $this->Mensagem = $EmailConteudo;
